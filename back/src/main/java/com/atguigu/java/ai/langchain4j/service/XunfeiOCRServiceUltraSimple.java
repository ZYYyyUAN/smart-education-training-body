...
/**
 * 科大讯飞OCR服务类 - 超简化版本
 * 只发送图片Base64数据，不包含任何额外字段
 */
@Slf4j
@Service
public class XunfeiOCRServiceUltraSimple {
    ...
    private String sendHttpRequest(String url, String imageBase64) throws IOException {
        ...
        try {
            ...
            // 超简化：只发送图片Base64字符串，不包装在任何JSON对象中
            String requestBody = "\"" + imageBase64 + "\"";
            log.info("请求体长度: {}", requestBody.length());
            log.info("请求体类型: 纯Base64字符串");
